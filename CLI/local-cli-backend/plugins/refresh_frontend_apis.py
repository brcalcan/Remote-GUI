...
def get_plugin_routes(plugin_name: str) -> List[Dict[str, Any]]:
    ...
    if not router_file.exists():
        print(f"❌ Plugin '{plugin_name}' not found: {router_file}")
        return []
    ...
    try:
        ...
        print(f"✅ Found {len(routes)} routes in {plugin_name}")
        for route in routes:
            print(f"   - {route['method']} {route['path']} -> {route['function_name']}")
        return routes
        
    except Exception as e:
        print(f"❌ Error parsing {plugin_name}: {e}")
        return []
...
def generate_frontend_api_file(plugin_name: str, routes: List[Dict[str, Any]]) -> str:
    ...
    """
    api_url = "window._env_?.VITE_API_URL || \"http://localhost:8000\""
...
def refresh_plugin_apis(plugin_name: str) -> bool:
    ...
    """
    print(f"🔄 Refreshing frontend APIs for '{plugin_name}'...")
    ...
    try:
        ...
        print(f"❌ Error writing {api_file}: {e}")
        return False
...
def refresh_all_plugins():
    ...
    print(f"🔄 Refreshing frontend APIs for {len(plugin_names)} plugins...")
    ...
    print(f"🎉 Successfully refreshed {success_count}/{len(plugin_names)} plugins!")
...
def main():
    ...
    print("🚀 Frontend API Refresh Tool")
    print("=" * 40)
    ...
    if args.plugin.lower() == 'all':
        ...
        if refresh_plugin_apis(args.plugin):
            print(f"🎉 Successfully refreshed '{args.plugin}' plugin!")
        else:
            print(f"❌ Failed to refresh '{args.plugin}' plugin")
            sys.exit(1)
